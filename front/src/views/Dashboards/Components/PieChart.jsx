...
const PieChart=()=>{
  ...
  const fetchdata  = async()=>{
    ...
    const response= await getPieChartData(startIso,endIso);
    setNumbers(response);
  }
  function filterByDates(dates) {
    ...
    return(
        <Card extra="!p-[20px] mt-[1%] ">
          ...
          Documents count per center
        </div>
          <TEDropdown className="flex justify-end pl-[30%]">
            <TEDropdownToggle>
            ...
            <Doughnut className="w-[600px] max-h-[400px] text-center" data={data} />
        </Card>
    );
}
